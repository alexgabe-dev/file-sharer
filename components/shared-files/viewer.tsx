...
export function FileViewer({ file, onClose, onShare, onDownload }: { file: SharedFile; onClose: () => void; onShare: () => void; onDownload: () => void }) {
  ...
  return <div role="dialog" aria-modal="true" aria-label={file.name} onKeyDown={(event) => { if (event.key === 'Escape') onClose() }} className="fixed inset-0 z-50 flex flex-col bg-primary/95 text-primary-foreground">
    <div className="flex h-16 items-center justify-between border-b border-primary-foreground/10 px-4 sm:px-8">
