...
export function UploadQueue({ items, onCancel, onRetry, onPause, onResume, onReselect, onPauseAll, onResumeAll, onRetryFailed, onCancelFailed }: { items: UploadItem[]; onCancel: (id: string) => void; onRetry: (id: string) => void; onPause: (id: string) => void; onResume: (id: string) => void; onReselect: (id: string) => void; onPauseAll: () => void; onResumeAll: () => void; onRetryFailed: () => void; onCancelFailed: () => void }) {
  ...
  return <section aria-label="Upload queue" className="mt-6 rounded-xl border border-border bg-card p-4">
    ...
    {items.length > 1 && <div className="mb-3 flex flex-wrap items-center gap-2">
      ...
      {failedCount > 0 && <button onClick={onRetryFailed} className="rounded-md border border-border px-2.5 py-1.5 text-xs font-medium text-accent hover:bg-secondary">Retry failed</button>}
    ...
    <div className="space-y-3">{items.map((item) => <div key={item.id} className="queue-item flex items-center gap-3"><div className="flex size-9 shrink-0 items-center justify-center rounded-md bg-secondary text-muted-foreground">{item.kind === 'video' ? <Film className="size-4" /> : item.kind === 'image' ? <ImageIcon className="size-4" /> : <File className="size-4" />}</div><div className="min-w-0 flex-1"><div className="flex justify-between gap-3 text-xs"><span className="truncate font-medium">{item.name}</span><span className="shrink-0 text-muted-foreground">{statusLabel(item)}</span></div>{showError(item) ? <p className="mt-1 truncate text-xs text-destructive">{item.error}</p> : <div className="mt-2 h-1 overflow-hidden rounded-full bg-secondary"><div className="progress-shine h-full rounded-full bg-accent transition-all duration-500" style={{ width: `${item.progress}%` }} /></div>}</div>{item.status === 'failed' ? <button onClick={() => onRetry(item.id)} aria-label={`Retry ${item.name}`} className="rounded-md px-2 py-1.5 text-xs font-medium text-accent hover:bg-secondary">Retry</button> : item.status === 'paused' ? <button onClick={() => onResume(item.id)} aria-label={`Resume ${item.name}`} className="rounded-md px-2 py-1.5 text-xs font-medium text-accent hover:bg-secondary">Resume</button> : item.status === 'reselect' ? <button onClick={() => onReselect(item.id)} aria-label={`Select ${item.name}`} className="rounded-md px-2 py-1.5 text-xs font-medium text-accent hover:bg-secondary">Select file</button> : showPause(item) ? <button onClick={() => onPause(item.id)} aria-label={`Pause ${item.name}`} className="rounded-md px-2 py-1.5 text-xs font-medium text-accent hover:bg-secondary">Pause</button> : null}{item.status !== 'complete' && <button onClick={() => onCancel(item.id)} aria-label={`Cancel ${item.name}`} className="rounded-md p-2 text-muted-foreground hover:bg-secondary hover:text-foreground"><X className="size-4" /></button>}</div>)}</div>
  </section>
}
